...
- target_audience is who the product is merchandised for, not who is modelling it. Take the merchant's value when supplied. Otherwise infer only from how the product itself is cut or constructed, and only when that is genuinely decisive. Never infer it from the appearance, body, or presentation of a person in the image. A product whose construction does not settle it is unisex when it is plainly worn by anyone, and otherwise null with status unknown.
