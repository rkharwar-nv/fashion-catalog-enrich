...
- target_audience is the department a product is merchandised under, not a statement about any person. Take the merchant's value when supplied. Otherwise infer only from how the product itself is cut or constructed, and only when that is decisive. Never infer it from the appearance, body, presentation, or perceived gender of a person in the image. Prefer adult_all_genders: use womens or mens only where the construction itself is specific, and adult_all_genders for adult sizing cut to be worn by anyone, which includes most accessories, bags, eyewear and jewellery. Use kids for children's sizing regardless of how it is cut; kids is not a subset of adult_all_genders. Where none of this settles it, return null with status unknown.
